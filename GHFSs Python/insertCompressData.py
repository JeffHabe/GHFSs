...
mypath ='D:\PythonWorkspace\PyExp\Data_csv\SlidingWindow\Chebyshev\Mean\Worst compressing data1110'
# =============================================================================
...
# =============================================================================
        ...
        with open(mypath+'\\'+filenames[fileIdx],newline='')as datafile:
            ...
            try:
                for row in spamreader:
                    ...
                    #print(strData)
                    value=[]
                    datetime_start = datetime.strptime(date+' '+strData[1], '%Y-%m-%d %H:%M:%S')
                    datetime_end = datetime.strptime(date+' '+strData[3], '%Y-%m-%d %H:%M:%S')
                ...
                try:
                    ...
                    print(date,'using data is',fileIdx,':',filenames[fileIdx][:-15])
    ...
    dfcolumns=['time','start_Date','end_Date','Compact Size','total days','count','fileIdx','filenames']
    if (pth.isfile('./Insertinto_Compressed_detailData.csv')!=True):  
        df.to_csv('Insertinto_Compressed_detailData.csv',mode='w',
                  columns=dfcolumns,
                  index=False)
    else:
        df.to_csv('Insertinto_Compressed_detailData.csv',mode='a',columns=dfcolumns,header=None,index=False)
